...
module.exports.run = async (bot, message, args) => {
  ...
  if(vip === true) amount = Math.floor(Math.random() * dens.maxParaVerme.dilencilikVIP) + 1;
  if (vip === null) amount = Math.floor(Math.random() * dens.maxParaVerme.dilencilik) + 1;
  ...
  .setDescription(`**Dostum Dilenerek \`${amount}\` ${dens.paraBirimi} kazandın vay aq**`);
