...
module.exports.run = async (bot, message, args) => {
  if(!message.content.startsWith(dens.prefix))return;  
    ...
    .setDescription(`**Para Sıralaması:** \`${dens.prefix}sıralama para\`\n**Nike Sıralaması:** \`${dens.prefix}sıralama nike\`\n**Araba Sıralaması:** \`${dens.prefix}sıralama araba\`\n**Ev Sıralaması:** \`${dens.prefix}sıralama ev\``)
  ...
  if(!args[0]) return message.channel.send(embed)
    ...
    if (args[0] == 'para') {
      ...
      let finalLb = "";
      for (var i in bakiye) {
        finalLb += `**${bakiye.indexOf(bakiye[i])+1}.**     <@${bakiye[i].ID.slice(25)}> - \`${bakiye[i].data} ${dens.paraBirimi}\`\n`;
      }
    ...
    const embed = new Discord.MessageEmbed()
    .setThumbnail(message.guild.iconURL({ dynamic: true }))
    .setDescription(`**${dens.altYazi} sunucun Nike Leaderboard**\n\n${finalLb}`)
    .setColor("#FFFFFF")
    .setFooter(dens.altYazi)
    message.channel.send(embed)
