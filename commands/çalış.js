...
module.exports.run = async (bot, message, args) => {
    ...
    if (author !== null && timeout - (Date.now() - author) > 0) {
        ...
        if(vip === true) amount = Math.floor(Math.random() * dens.maxParaVerme.calismaVIP) + 1;
        if (vip === null) amount = Math.floor(Math.random() * dens.maxParaVerme.calisma) + 1
        let embed1 = new Discord.MessageEmbed()
        .setColor("#FFFFFF")
        .setTitle(message.author.tag)
        .setFooter(dens.altYazi)
        .setDescription(`\`${replies[result]}\` **Olarak çalıştın ve karşılığında \`${amount}\` ${dens.paraBirimi} kazandın!**`);
